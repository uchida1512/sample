...
class CanNotSubtractTypeError(ValueError):
    pass


class CanNotSubtractNNumberError(ValueError):
    pass
...
class ABC:
    ...
    def sub(self) -> int:
        if self._x_or_y_is_not_int():
            raise CanNotSubtractTypeError('整数を入力してください')
        elif self._x_or_y_is_negative_number():
            raise CanNotSubtractNNumberError('正数を入力してください')
